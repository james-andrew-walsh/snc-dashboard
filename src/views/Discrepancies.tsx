...
function sortKey(row: Anomaly): number {
  const statusBase = (STATUS_ORDER[row.anomalyType] ?? 3) * 10
...
export function Discrepancies() {
  ...
  useEffect(() => {
    async function fetchData() {
      const [anomalyRes, siteLocRes] = await Promise.all([
        supabase.from('Anomaly').select('*, SiteLocation(name)').is('resolvedAt', null),
      ...
      // Group anomalies by siteLocationId
      const grouped = new Map<string, Anomaly[]>()
      for (const row of anomalies) {
        const siteId = row.siteLocationId
      ...
      const siteGroups: SiteGroup[] = geofenced.map(loc => {
        ...
        return {
          siteName: loc.name,
          rows,
          anomalyCount: rows.filter(r => r.anomalyType === 'ANOMALY_NO_HJ').length,
          disputedCount: rows.filter(r => r.anomalyType === 'DISPUTED').length,
          notInEitherCount: rows.filter(r => r.anomalyType === 'NOT_IN_EITHER').length,
        }
      })
          ...
          <div className="px-5 py-4 border-b border-slate-700 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 text-xs">
              {group.anomalyCount > 0 && (
                <span className="text-red-400">{group.anomalyCount} anomal{group.anomalyCount !== 1 ? 'ies' : 'y'}</span>
              )}
                    ...
                    <tr key={row.equipmentCode} className="hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="px-5 py-2.5">
                        <span className={`inline-flex items-center gap-1.5 ${row.engineStatus === 'Active' ? 'text-green-400' : 'text-slate-500'}`}>
                          ...
                          {row.engineStatus ?? 'Off'}
                        </span>
                      </td>
                      <td className="px-5 py-2.5 font-mono text-slate-300">{row.e360JobCode ?? '—'}</td>
                      <td className="px-5 py-2.5 text-slate-300">
                        {(row.anomalyType === 'ANOMALY_NO_HJ' || row.anomalyType === 'DISPUTED')
                          ? (row.e360LocationName ?? '—')
                          : '—'}
                      </td>
                      <td className="px-5 py-2.5 text-slate-300">
                        {row.anomalyType === 'DISPUTED'
                          ? <span><span className="font-mono">{row.hjJobCode ?? '—'}</span>{row.hjJobDescription ? <span className="text-xs text-slate-400 ml-1">— {row.hjJobDescription}</span> : null}</span>
                          : '—'}
                      </td>
                      <td className="px-5 py-2.5 font-mono text-slate-300">
                        {row.hourMeter != null ? `${Math.round(row.hourMeter).toLocaleString()} hrs` : '—'}
                      </td>
                      <td className="px-5 py-2.5">
                        <span className={`inline-block text-[11px] font-semibold uppercase px-2 py-0.5 rounded border ${statusColor(row.anomalyType)}`}>
                          {statusLabel(row.anomalyType)}
                        </span>
                      </td>
                    </tr>
                  ))}
